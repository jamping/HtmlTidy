...
public:
  Source()
  {
    getByte    = reinterpret_cast<TidyGetByteFunc>(get);
    ungetByte  = reinterpret_cast<TidyUngetByteFunc>(unget);
    eof        = reinterpret_cast<TidyEOFFunc>(end);
    sourceData = reinterpret_cast<void*>((ulong) this);
  }
  virtual ~Source() {}
...
public:
  Sink()
  {
    putByte  = reinterpret_cast<TidyPutByteFunc>(put);
    sinkData = reinterpret_cast<void*>((ulong) this);
  }
  virtual ~Sink() {}
...
public:
    ...
    int  Create()
    {
        ...
        if ( _tdoc = tidyCreate() )
        {
            tidySetAppData( _tdoc, reinterpret_cast<void*>((ulong) this ));
//            tidySetReportFilter( _tdoc, ReportFilter );//reinterpret_cast<TidyReportFilter>(ReportFilter) );
            return 0;
        }
        return -1;
    }
    ...
    */
    void  SetAppData( ulong data ) { tidySetAppData( _tdoc, reinterpret_cast<void*>(data) ); }
    ulong GetAppData()             { return reinterpret_cast<ulong>(tidyGetAppData( _tdoc )); }
...
protected:
    ...
    */
    static Bool ReportFilter( TidyDoc tdoc,TidyReportLevel lvl, uint line, uint col, ctmbstr msg )
    {
