...
// delete the node
TidyNode TIDY_CALL    tidyNodeRemove( TidyNode tnod )
{
	Node* nimp = TY_(RemoveNode)(tidyNodeToImpl( tnod ));
	return tidyImplToNode(nimp);
}
TidyNode TIDY_CALL  tidyNodeDiscard( TidyDoc tdoc, TidyNode tnod )
{
	Node* nimp = NULL;
	TidyDocImpl* doc = tidyDocToImpl( tdoc );
	if ( doc )
		nimp = TY_(DiscardElement)( doc, tidyNodeToImpl(tnod) );
